...
use app\controllers\access\AssignmentController;
use Yii;
use yii\base\Model;
...
class AuthItem extends Model
{
        ...
        if (!Configs::authManager()->getRule($name)) {
            try {
                ...
                if ($rule instanceof Rule) {
                    ...
                    $this->addError('rule_name', Yii::t('app', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('rule_name', Yii::t('app', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            $this->_item->rule_name = $this->rule_name;
